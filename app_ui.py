...
# --- 4. ממשק מחובר ---
    ...
    if st.session_state['role'] == 'teacher':
        ...
        with t_approve:
            ...
            status_filter = col_filter.selectbox("סנן לפי סטטוס:", ["הכל", "מאושר", "לתיקון", "הוגש"])

            df_hist = df_subs.copy()
            if status_filter != "הכל":
                df_hist = df_hist[df_hist['סטטוס'].str.strip() == status_filter]
            if search_query:
        ...
        for s in all_stages:
            ...
            if sub_at_stage.empty:
                curr_stage, curr_stat = s, "";
                break
            else:
                last_stat = str(sub_at_stage.iloc[-1]['סטטוס']).strip()
                if last_stat != "מאושר":
                    curr_stage, curr_stat = s, last_stat;
                    break
